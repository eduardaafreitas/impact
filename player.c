...
void init_bullet(bullets *bullet) {
    bullet->pos_x = -1;    // Posição inicial fora da tela
    bullet->pos_y = -1;    // Posição inicial fora da tela
    bullet->speed = 8.0;  // Velocidade do tiro
    bullet->active = false; // Marca como inativo
}
...
player_ship *init_player(ALLEGRO_BITMAP* sheet){
    ...
    for (int i = 0; i < MAX_BULLETS; i++) {
        init_bullet(&player->bullet[i]); // Inicializa cada tiro individualmente
    }
